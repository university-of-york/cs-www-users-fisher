...
#define MAXINTERP   16384 /* must be <= 2^15; any space above power of 2 is wasted */
...
static roundup()
  { seqlen = ndata;
	...
	if (seqlen > ndata)
	  { if (seqlen > MAXINTERP-1) /* we need one extra cell for interpolation */
	      { fprintf(stderr, "fft: too many data for interpolation! max = %d\n", MAXINTERP-1);
		exit(1);
	      }
	    interpolate();
	  }
