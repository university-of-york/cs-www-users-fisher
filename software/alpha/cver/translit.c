...
#define readbitmap(fn)	ReadBitmap(NULL, fn, 0)
#define bfree(bm)	Bfree(NULL, bm)
#endif

#ifdef sun
#define MKBOX		"/usr/fisher/sunbin/mkbox"
#define readbitmap(fn)	ReadBitmap(fn)
#define bfree(bm)	Bfree(bm)
#endif
...
struct box
  { struct box *link;
    int wid, hgt, lth;	/* in points */
    char fn[16];
  };

static struct box *boxlist;
...
global int boxtranslit(wid, hgt, lth) int wid, hgt, lth;
  { char trv[MAXSTRLEN+1]; struct box *box;
    wid = (wid+500)/1000; hgt = (hgt+500)/1000; lth = (lth+500)/1000; /* cvt to points */
    box = boxlist;
    until (box == NULL || (box -> wid == wid && box -> hgt == hgt && box -> lth == lth)) box = box -> link;
    if (box == NULL)
      { /* box file does not exist; make it */
	box = heap(1, struct box);
	box -> wid = wid; box -> hgt = hgt; box -> lth = lth;
	makebox(box);
	box -> link = boxlist; boxlist = box; /* link into list */
      }
    sprintf(trv, "%c\"%s\"", (pmode ? 'p' : 'b'), box -> fn);
    return lookuptranslit(lookupword(trv));
  }

static makebox(box) struct box *box;
  { /* file does not exist; make it */
    char cmd[MAXSTRLEN+1]; int code;
    strcpy(box -> fn, "/tmp/box.XXXXXX"); mktemp(box -> fn);
    sprintf(cmd, "%s -%c %d %d %d >%s", MKBOX, (pmode ? 'p' : 'b'),
	    box -> wid, box -> hgt, box -> lth, box -> fn);
    code = system(cmd);
  ...
  { int tc = ERROR_TCHAR, i = numtrs-1;
	...
	if (ok)
	  { int ft, ch;
	    ...
	    ft = tc >> 8; ch = tc & 0377;
	    fontinfo[ft] -> advance[ch].x = trlit.wid; /* set width in fontinfo entry of new char */
...
static int index_to_tc(n) int n;
  { int f = 0;
    while (n >= NUMTCHARS) { f++; n -= NUMTCHARS; }  /* select font */
    return (f << 8) + (n + MINTCHAR);
  }
