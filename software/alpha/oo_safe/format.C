...
enum direct
  { /* < > directives */
    ...
    d_head, d_foot, d_fn, d_use, d_macro,
    /* backslash directives */
    b_comment, b_glue, b_horiz, b_vert, b_tab, b_fix, b_tabfix,
    b_line, b_need, b_skip, b_page, b_arg, b_tr, b_box, b_indent,
...
static int context, symtop, bsbase, dirbase, macrobase, numtlits, minline, maxline, linenum, linesskipped;
...
static tlit tlittab[MAXTLITS];

static void declsyswords(), dopass(node*), setbasicfonts(), formatseq(), fmtseq();
...
static void bsdir(symtabent*), tabulate(), fixindent();
static node *collectspan();
static char *collectword(), *substfordollars(char*);
static node *findarg(int);
static void badarg(int), fmtword(char*, int), addspaces(char*, char*, int), decodeescapes(char*, line*);
static void decodetlits(line*, line*), stuffpagenum(int, line*), stuffpageroman(int, line*), stuffpageletter(int, line*);
static int matchtlit(line*, int, int, line*);
static void copytr(line*, int, int, line*);
static bool stuffligature(int, Char*, line*);
static void stuffindex(int, line*);
...
global void initfmtfile()
  { if (firstfile)
    ...
    outproc(o_bofile);
    inittranslit();
    for (int i=0; i < FONTSIZE/BPW; i++) tlitbits[i] = 0;
    currpage = currfont = -1;
    generation = 0;
...
static void blkdir(symtabent *x)
  { switch (x -> dval)
	...
	case d_def:
	  { int st = symtop;
	    char *arg1 = collectword();
	    int nargs;
	    if (ftext -> typ == s_word)
	      { char *arg = collectword();
	...
	case d_tr:
	  { int nt = numtlits, tk = -1; uint tb;
	    char *w1 = collectword();
	    char *w2 = (ftext != NULL && ftext -> typ == s_word) ? collectword() : NULL;
	    if (w1[0] == '\\') w1 = lookupword(&w1[1]); /* remove leading backslash if any */
	    if (numtlits < MAXTLITS)
	      { tlit *tl = &tlittab[numtlits];
		tl -> len = strlen(w1);
		tl -> tv = w1;
		tl -> su = (w2 != NULL) ? symtranslit(w2) : NULL;
		if (w1[0] != '\0')
		  { if (w2 != NULL)
		      { /* set bit in bitmap corresponding to first char */
			int fc = w1[0] & 0377;
			tk = fc/BPW; tb = tlitbits[tk];
			setbit(tlitbits, fc);
		      }
		    numtlits++;
		  }
  ...
  { switch (x -> dval)
	...
	case b_tr:
	  { char *arg = collectword();
	    ...
	    unless (sub == NULL) stuffligature(fonts[Roman], sub, &oline);
	    break;
	  }

	case b_box:
	  { char *arg; int wid, hgt;
	    arg = collectword();	/* width  */
	    setnumber(arg, wid, n_unitsok);
	    arg = collectword();	/* height */
...
static char *collectword()
  { node *arg = ftext;
    ...
    ftext = ftext -> fwd;
    char *s = arg -> wrd;
    if (version >= 4 && firstarg != NULL) s = substfordollars(s);
    return s;
  }

static char *substfordollars(char *s)
  { char v[MAXSTRLEN+1]; int j = 0, k = 0; bool any = false;
    until (s[j] == '\0')
...
static void fmtword(char *w, int nsp)
  { char s[MAXSTRLEN+1]; line v1, v2;
    if (version >= 4 && firstarg != NULL) w = substfordollars(w);
    addspaces(w, s, nsp);
    decodeescapes(s, &v1);
    decodetlits(&v1, &v2);
...
static void decodetlits(line *v1, line *v2)
  { int i = 0;
	...
	int matched = 0;
	if (c.ft > 0)
	  { if ((vertoffset == headposn || vertoffset == footposn) && (c.ch == '#' || c.ch == '%' || c.ch == '$'))
	    ...
	    else if (testbit(tlitbits, c.ch)) matched = matchtlit(v1, i, v1 -> len - 1, v2);
	  }
	if (matched == 0)
	  { v2 -> stuffc(v1 -> vec[i]); /* no match, copy input char */
	    matched = 1;
	  }
...
inline bool ismatch(char tc, Char vc)
  { /* There is a match between a template char and an actual (vector) char if:
    ...
    return (vc.ft > NUMTFONTS) && (vc.ch == tc);
  }

static int matchtlit(line *v1, int p1, int p2, line *v2)
  { /* search list of translits, find longest which matches v1[p1..p2], stuff it into v2 */
    int maxj; int len = 0;
    for (int j=0; j < numtlits; j++)
      { int tl = tlittab[j].len;
	if (tl >= len && p1+tl <= p2+1) /* N.B. ``>='' so we find most local one */
	  { char *tv = tlittab[j].tv;
	    Char *iv = &v1 -> vec[p1];
	    until ((*tv == '\0') || !ismatch(*tv, *iv)) { tv++; iv++; }
	    if (*tv == '\0') { len = tl; maxj = j; }
	  }
      }
    if (len > 0)
      { Char *sub = tlittab[maxj].su;		    /* "word2" of longest matching transliteration */
	if (sub == NULL) copytr(v1, p1, len, v2);   /* no subst def'd; copy input chars unchanged */
	else
	  { bool ok = stuffligature(v1 -> vec[p1].ft, sub, v2);
	    unless (ok)
...
static void copytr(line *v1, int p1, int len, line *v2)
  { for (int i=0; i < len; i++) v2 -> stuffc(v1 -> vec[p1+i]);
  }

static bool stuffligature(int cft, Char *u, line *v)
  { int dx = 0;
	...
	if (c == LIGUPCH || c == LIGDNCH)
	  ...
	  { int ft = u[k].ft;
	    ...
	    if (ft < NUMTFONTS || testbit(fontinfo[ft] -> defd, c))
	      { Char lc = Char(ft, c);
		int cw = lc.width();
		if (!any) fw = cw; /* width of first char (for centring) */	    /* WAS (k == 0) */
		dx += (fw-cw)/2;
		if (dx != 0)
...
static void outputline(bool just)
  { if (linenum >= minline && linenum <= maxline)
	...
	for (int i=0; i < oline.len; i++)
	  { Char c = oline.vec[i];
	    unless (c.ft == currfont || c.ch == GLUECH)
