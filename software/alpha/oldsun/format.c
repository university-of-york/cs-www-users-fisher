...
#define MAXCONTEXT   80	    /* max. depth of nesting of begin-end brackets		   */
...
#define f_reformat   0x10   /* begin bracket edited; reformat all below */

enum direct
  { /* < > directives */
    d_bit, d_def, d_text, d_null, d_ft, d_ls, d_in, d_rin, d_tabs, d_tr,
...
struct tlit
  { int len;	/* length of sequence to transliterate			     */
    ...
    int u;	/* single char in font 0, or (LIGBIT | [c1:c2:c3])	     */
  };
...
extern initfonts(), splitfontname(), setnumber();	    /* from common   */
...
static int vertoffset, linewid, gluecount, supergluecount, linespacing, indent, nextindent, rightindent, currpage;
...
static declsyswords()
  { /* declare backslash directives */
    ...
    declare("<ls", d_ls);                declare("<in", d_in);
    declare("<rin", d_rin);              declare("<tabs", d_tabs);
...
global initfmtfile()
  { int i;
	...
	int tf;
	outproc(o_bobook);
	tf = lookupfont("C", 12); /* transliteration font */
...
static setbasicfonts()
  { int k;
    for (k=0; k < NUMFP; k++) fonts[k] = NULL;
    setfonts(Roman,  "C",  12);
    setfonts(Italic, "CO", 12);
    setfonts(Bold,   "CB", 12);
    for (k=0; k < NUMFP; k++)
      if (fonts[k] == NULL) a_giveup("can't find basic font (position %d)!", k);
  }
	...
	case s_word:
		...
		if (w[0] == '\\')
		  { if (testbit(escapable, w[1] & 0377))
		      { formatword(&w[1], ftext -> spb); /* remove leading backslash */
			ftext = ftext -> fwd;
		      }
		    else if (w[1] >= '0' && w[1] <= '9')
		      { int n;
...
static bsdir(x) struct symtabent *x;
  { switch (x -> dval)
	...
	case b_glue:
	    ...
	    if (oline[0] < MAXSTRLEN)
	      { stuffc(SUPERGLUECH, oline);
		linewid += mingluewidth;
		supergluecount++;
	      }
	    break;
	...
	case b_horiz:	case b_vert:
	  { /* horizontal space */
	    ...
	    if (oline[0] < MAXSTRLEN)
	      { stuffc(c, oline); /* font 0, char c */
		if (x -> dval == b_horiz) linewid += d;
	      }
	    break;
	  }
...
static formatword(w, nsp) char *w; int nsp;
  { char s[MAXSTRLEN+1];
...
static decodeescapes(s, v) char *s; int *v;
  { int i = 0, index = 0; int ch;
	...
	unless (ch == '\0')
	  { unless (cf) stuffc((fonts[kf] << 8) + (ch & 0377), v);
	    ch = s[i++];
	  }
	continflag |= cf;
      }
  ...
  { /* search list of translits, find longest which matches v1[p1..p2], stuff it into v2 */
	...
	if (tl >= len && p1+tl <= p2+1) /* N.B. ``>='' so we find most local one */
	  { char *tv = tlittab[j].tv;
	    int *iv = &v1[p1];
	    until ((*tv == '\0') || ((*tv & 0377) != (*iv & 0377))) { tv++; iv++; }
	    if (*tv == '\0')
	      { len = tl;
		maxj = j;
	      }
	...
	else stuffc(u, v2);	/* it's a transliteration (statically bound font); use font zero */
      }
    return len;
  }
  ...
  { int dx = 0;
	...
	if (c != '\0' && testbit(fontinfo[lft] -> defd, c))
	  { int cw = fontinfo[lft] -> advance[c].x;
	    ...
	    stuffc((lft << 8) + c, v); /* implicit move right cw millipoints */
	    dx -= (fw+cw)/2;
	    any = true;
	  }
      }
...
static outputline(just) bool just;
  { if (linenum >= minline && linenum <= maxline)
    ...
    indent = nextindent;
    oline[0] = 0; linewid = gluecount = supergluecount = 0;
    linenum++;
