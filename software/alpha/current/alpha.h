...
struct node	/* text record */
  { node(nodetype, char*, int);		/* constructor */
    ...
    nodetype typ;
    char *wrd;
    node *fwd, *bwd, *xfwd, *xbwd, *slk, *unm;
    int lev, spb, spn, skp, vof, gln;
    bool edt;
  };
...
struct tlit
  { char *tv;		/* sequence to transliterate	 */
    int len;		/* length of tv			 */
    void *sub;		/* substitution (char* or Char*) */
    bool used;
  };
...
struct substitution
  { substitution();
    ...
    void read(char*);
    tlit *lookup(char*);
    void rpt_unused(), write(char*);
    void add(char*, void*);
    void match(char*, char*, bool&);

    tlit *tab;
    int max, num;
    uint bits[FONTSIZE/BPW];
  };
...
extern bool pmode, epsflag, rpt_unusedflag;			/* from main	  */
...
extern void initeditfile(), processfile();			/* from edit	  */
extern bool isbrac(node*);					/* from edit	  */
