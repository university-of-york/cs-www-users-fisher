...
global bool pmode, epsflag, rpt_unusedflag;
...
static char tempfn[18];

static void newhandler(), readcmdline(char*[]), usage();
static void processbook(), copyfile(FILE*, FILE*), unlinktemp(), closefiles();
...
static void readcmdline(char *argv[])
  { int ap = 0; bool minmaxset;
    ...
    pmode = epsflag = rpt_unusedflag = minmaxset = false;
	...
	else if (seq(argv[ap],"-u"))
	  { rpt_unusedflag = true;	/* report unused xrefs */
	    ap++;
	  }
...
static void usage()
  { fprintf(stderr, "Usage: alpha fn          [-Dopt ...] [-fp n] [-u]\n");
    fprintf(stderr, "       alpha [fn ...] -p [-Dopt ...] [-fp n] [-pg m [n]] [-e] [-u]\n");
    exit(1);
  }
  ...
  { for (int k = 0; k < numfnames; k++)
	...
	if (fn == NULL)
	  { /* copy stdin to temp file */
	    strcpy(tempfn, "/tmp/alpha.XXXXXX"); mktemp(tempfn);
	    atexit(unlinktemp);
	    FILE *fi = fopen(tempfn, "w");
	    if (fi == NULL) giveup("can't create %s", tempfn);
	    copyfile(stdin, fi);
	    fclose(fi);
	    fn = tempfn;
	  }
...
static void copyfile(FILE *ifi, FILE *ofi)
  { int ch = getc(ifi);
    until (ch < 0) { putc(ch, ofi); ch = getc(ifi); }
  }

static void unlinktemp()
  { unlink(tempfn);
  }
