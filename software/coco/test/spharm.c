...
static makederivs()
  { int j, k;
    ...
    av[2] = -MEPS2; av[4] = +MEPS4; av[6] = -MEPS6;
    linterm[0] = MEPSK;
    for (j=1; j < NDERIVS; j++)
  ...
  { int k;
	...
	for (i=0; i < NDERIVS-1 && !cvg; i += 2)
	  { double at, bt, gt, dt;
	    at = term * avec[i][k];
	    bt = term * bvec[i][k];
	    gt = lam * term * avec[i+1][k];
	    dt = lam * term * bvec[i+1][k];
	    asum += at; bsum += bt; gsum += gt; dsum += dt;
	    if (fabs(at) + fabs(bt) + fabs(gt) + fabs(dt) < EPS) cvg = true;
	    term *= -(lam*lam);
	  }
...
static ft_convert(pc, cc) struct pco *pc; struct cco *cc;
  { double phi, xsum, ysum; int k;
    phi = pc -> lat * RADIANS;
    xsum = 0.0; ysum = linterm[0]*phi;
    for (k=0; k < NHARMS; k++)
      { xsum += gamma[k]*sin(k*phi) + delta[k]*cos(k*phi);
	ysum += alpha[k]*sin(k*phi) + beta[k]*cos(k*phi);
      }
    cc -> x = X0 + (AIRY_A*SCALE) * xsum;
    cc -> y = Y0 + (AIRY_A*SCALE) * ysum - M0;
  }
