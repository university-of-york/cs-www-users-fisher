...
static printderivs()
  { int i, j;
    ...
    printf("global int linterm = %d;\n\n", ifix(linterm[0] * ITWOPI));
    printf("global int cvec[%d][%d] = {\n", NDERIVS, NHARMS/2);
    for (i=0; i < NDERIVS; i++)
      { double *vec = (i&1) ? &bvec[i][1] : &avec[i][0];
	for (j=0; j < NHARMS; j += 2)
	  { if (j%8 == 0) printf((j == 0) ? "  { " : "    ");
	    printf("%d, ", ifix(FXSCALE * vec[j]));
	    if (j%8 == 6) putchar('\n');
	  }
	printf("  },\n");
      }
...
static int ifix(x) double x;
  { double top = - (double) (1 << 31);	/* top is +ve */
    if (x > top-1 || x < -top)
      { fprintf(stderr, "mktable: ifix o'flow: %g\n", x);
	exit(1);
      }
